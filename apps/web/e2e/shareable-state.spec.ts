...
// Render, and a link to a Pearl realm opened on Mainnet — showing a
...
test("a link can name the network, and it wins over the stored one", async ({ page }) => {
  ...
  await expect(page.locator("#window-settings select")).toHaveValue("pearl");
...
test("the network survives navigating around inside the app", async ({ page }) => {
  ...
  await page.setViewportSize({ width: 1500, height: 900 });
  await page.goto("/?net=pearl&pkg=gno.land/r/sys/users");
  await page.waitForSelector("#window-realm");
  expect(new URL(page.url()).searchParams.get("net")).toBe("pearl");
